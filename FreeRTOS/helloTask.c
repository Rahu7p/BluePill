...
/**
  ...
  ******************************************************************************
  * This program creates one Task using the CMSIS_V1 API (FreeRTOS).
  * The Task prints every one second using the UART protocol.
  *
  ******************************************************************************
  */
...
int main(void)
{
  ...
  osThreadDef(TaskHandle1, Task1, osPriorityNormal, 0, 128);
  TaskHandle1 = osThreadCreate(osThread(TaskHandle1), (void *)aTask1);
...
{
  /* USER CODE BEGIN 5 */
  int i = (int)(void *)argument;
  /* Infinite loop */
  for(;;)
  {
	  myprintf("Hello Task %d\n", i );
	  osDelay(1000);
  }
  /* USER CODE END 5 */
}
